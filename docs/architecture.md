...
- **cid 是多分 P 的必需区分项**：`bvid` 单独会合并分 P，`bilibili:<bvid>:<cid>` 才能定位到具体分 P；
- **本地播放靠路径**：`local:` 前缀与 `LocalTrackId.path` 解析，路径即身份；
...
| **M2 曲库** | 本地扫描 + 在线元数据入库 + 统一列表/搜索 | 混合列表可查询，FTS 生效 | 已完成 |
...
| **M6 打磨** | 自适应、设置、错误处理、增量扫描、CI/发布 | 桌面/移动自适应无布局问题 | 已完成（歌词 / 登录 / 应用图标另行安排） |
